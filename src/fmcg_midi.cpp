...
std::vector<FrameStats> ScaleMidiProcessor::process_midi(
    ...
    int parse_threads, bool track_dedup)
{
    out_total_ticks = 0;
...
{
    uint64_t total_ticks = 0;   // discarded
    return process_midi(filename, fps, out_division, out_total_notes, vel0_as_note_off,
                        total_ticks, cb);
}
